/*
     ...
     Shows how to read a string without showing it on the screen.
...
int main( void )
{
     ...
     data.c_lflag = ISIG | ICANON | ECHOE | ECHOK | ECHONL | IEXTEN;
     ...
     ret = read_stdin( pass, 128, "Password: " );
     if ( ret != 0 )
     {
          ret = errno;
          printf( "Something went wrong. (4)" );
          if ( ret != 0 )
          {
               printf( "  Error: %s.\n", strerror( ret ) );
          }
